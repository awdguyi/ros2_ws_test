...
class PlainPolygon(PlainGeometry):
    ...
    @classmethod
    def from_list_of_tuples(cls, vertices: List[tuple[float, float]]):
...
class PlainEllipse(PlainGeometry):
    ...
    def return_polygon_approximation(self, n:int=10) -> PlainPolygon:
        ...
        rotation_matrix = [[math.cos(a), -math.sin(a)], 
                           [math.sin(a), math.cos(a)]]
        ellipse_samples_ = [(rx*math.cos(2*math.pi*i/n), ry*math.sin(2*math.pi*i/n)) for i in range(n)]
        ellipse_samples = [PlainPoint(x + rotation_matrix[0][0]*sample[0] + rotation_matrix[0][1]*sample[1], 
                                      y + rotation_matrix[1][0]*sample[0] + rotation_matrix[1][1]*sample[1]) for sample in ellipse_samples_]
...
if __name__ == "__main__":
    ...
    list_of_points_ = [(1,2), (1,1), (1,0), (0,1)]

    list_of_points = [PlainPoint(*v) for v in list_of_points_]
