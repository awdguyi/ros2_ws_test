...
def generate_launch_description():
    ...
    # paused=true  → pass --pause flag (Gazebo starts paused)
    # paused=false → no --pause flag  (Gazebo starts running)
    gazebo_paused = ExecuteProcess(
        cmd=['gazebo', '--verbose', world_file_path,
             '-s', 'libgazebo_ros_init.so', '-s', 'libgazebo_ros_factory.so', '--pause'],
        output='screen',
    ...
    ld.add_action(gazebo_paused)
    ld.add_action(gazebo_running)
