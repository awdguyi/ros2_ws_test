...
def generate_launch_description():
    ...
    ### Fixed Values ###
    # robot_1_init_state = [1.0, -2.2, math.pi/2] # x y theta
    # robot_2_init_state = [1.0, 12.3, -math.pi/2] # x y theta
    robot_1_init_state = [8.5, -5.4, math.pi/2] # x y theta
    robot_2_init_state = [8.5, 12.3, -math.pi/2] # x y theta

    ### Default Values ###
    default_timer_period = 0.2

    default_robot_id_list = ['0', '1']
    default_robot_namespace_list = ['mir'+str(i) for i in default_robot_id_list]
    ...
    default_schedule_file_name = 'warehouse_schedule_dual.csv'

    default_config_mmp_fname = 'wsd_1t20_poselu_enll_train.yaml'

    default_rviz_file_name = 'mmp_dual_mir_robot_surroundings.rviz'
    default_world_file_name = 'aws/small_warehouse_3.world'
    ...
    for r_init, r_ns in zip([robot_1_init_state, robot_2_init_state], 
        ...
        robot_launch_list = summon_robot(pkg_share_robot, r_ns, r_init)
    ...
    pkg_name_6 = 'zmr_mpc'
    pkg_share_mpc = FindPackageShare(package=pkg_name_6).find(pkg_name_6)

    launch_file_6_1 = PythonLaunchDescriptionSource(
        [pkg_share_mpc, '/launch/zmr_mpc.launch.py'])
    include_launch_6_1 = IncludeLaunchDescription(launch_file_6_1, launch_arguments={
        'timer_period': timer_period,
        'enable_fleet_manager': enable_fleet_manager,
        'robot_namespace': robot_1_namespace,
        'config_robot_fname': config_file_name,
        'robot_id': robot_1_id}.items()
    )
    ld.add_action(include_launch_6_1)

    launch_file_6_2 = PythonLaunchDescriptionSource(
        [pkg_share_mpc, '/launch/zmr_mpc.launch.py'])
    include_launch_6_2 = IncludeLaunchDescription(launch_file_6_2, launch_arguments={
        'timer_period': timer_period,
        'enable_fleet_manager': enable_fleet_manager,
        'robot_namespace': robot_2_namespace,
        'config_robot_fname': config_file_name,
        'robot_id': robot_2_id}.items()
    )
    ld.add_action(include_launch_6_2)
