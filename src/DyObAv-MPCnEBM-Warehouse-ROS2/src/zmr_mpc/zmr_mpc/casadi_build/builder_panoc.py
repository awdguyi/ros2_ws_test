...
class PanocBuilder:
    ...
    def step_cost(self, step_in_horizon: int, action: ca.SX, last_state: ca.SX,
        ...
        if kt < critical_step: # (predicted) robot at the kt-th step should avoid the dynamic obstacles at their current positions
            x_dyn_cur     = dynamic_obstacles[0::self._cfg.ndynobs*(self.N_hor+1)]
            y_dyn_cur     = dynamic_obstacles[1::self._cfg.ndynobs*(self.N_hor+1)]
            rx_dyn_cur    = dynamic_obstacles[2::self._cfg.ndynobs*(self.N_hor+1)]
            ry_dyn_cur    = dynamic_obstacles[3::self._cfg.ndynobs*(self.N_hor+1)]
            alpha_dyn_cur = dynamic_obstacles[5::self._cfg.ndynobs*(self.N_hor+1)]
            # Stage 2: tight safety Gaussian for near-term current-position cost.
            # Small sigma keeps it as a safety net, not a wide steering force.
            _sigma_cur = 0.8
            cts.cost_dynobs += mc.cost_gaussian_obstacles(
                state, x_dyn_cur, y_dyn_cur, alpha_dyn_cur,
                sigma=_sigma_cur, weight=self._large_weight)
