...
# Custom import 
from .configs import MpcConfiguration, CircularRobotSpecification # type: ignore
from .cost_monitor import CostMonitor, MonitoredCost # type: ignore
...
class TrajectoryTracker:
    ...
    def set_work_mode(self, mode:str='safe', use_predefined_speed:bool=True):
        ...
        if mode == 'aligning':
            ...
            # Scale obstacle weights down in aligning mode to break v=0 local minimum.
            # Ratio drops from ~200:1 to ~60:1 (qdynobs_eff:qvel_eff), still strongly avoids obstacles.
            self.stc_weights = [self.config.qstcobs * 0.3] * self.N_hor
            self.dyn_weights = [self.config.qdynobs * 0.3 * (0.5 ** kt) for kt in range(self.N_hor)]
        else:
            self.tuning_params = [self.config.qpos, self.config.qvel, self.config.qtheta, self.config.lin_vel_penalty, self.config.ang_vel_penalty,
                                  self.config.qpN, self.config.qthetaN, self.config.qrpd, self.config.lin_acc_penalty, self.config.ang_acc_penalty]
            self.stc_weights = [self.config.qstcobs] * self.N_hor
            self.dyn_weights = [self.config.qdynobs * (0.5 ** kt) for kt in range(self.N_hor)]
            if use_predefined_speed:
                if mode == 'safe':
