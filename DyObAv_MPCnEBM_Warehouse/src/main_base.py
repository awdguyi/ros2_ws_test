...
from pkg_mp_singulartraj.motion_predict import MotionPredictor as MotionPredictorST
...
class MainBase:
    ...
    def _prepare_motion_predictor(self, type, config_file_path, ref_image_path, model_suffix='0'):
        ...
        if type is None:
            ...
            self.motion_predictor = MotionPredictorST(config_file_path)
        else:
    ...
    def run_motion_prediction(self, eps=0.5, min_sample=10, enlarge=3.0, extra_margin=0.0, debug=False):
        ...
        elif self.motion_predictor_type == 'st':
            assert isinstance(self.motion_predictor, MotionPredictorST)
            all_past_traj = [human.past_traj for human in self.humans]
            pred_samples_all_list = self.motion_predictor.get_motion_prediction_samples(
                all_past_traj, num_samples=20,
                sampling_time=self.config_robot.ts, pred_len=self.config_tracker.N_hor)
            pred_samples_all = [np.concatenate([x[i] for x in pred_samples_all_list], axis=0) for i in range(len(pred_samples_all_list[0]))]
...
if __name__ == '__main__':
    ...
    elif predictor_type == 'st':
        # SingularTrajectory: path to per-scene JSON config; checkpoint is derived automatically.
        # Default: hotel scene (closest to warehouse/indoor environment).
        motion_predictor_config_path = os.path.join(
            '/home/michael/ai_training_ws/SingularTrajectory',
            'config', 'stochastic',
            'singulartrajectory-transformerdiffusion-hotel.json')
    else:
