...
class MpcControllerNode(Node):
            ...
            try:
                ...
                for Tt, (mu_list, std_list, conf_list) in enumerate(
                    ...
                    for Nn, (mu, std, conf) in enumerate(
                        ...
                        s = dir_scales[Nn] if Nn < len(dir_scales) else 1.0
                        std = (max(std[0], 0.15) * s, max(std[1], 0.15) * s)
                        dyn_obs_list[Nn][Tt] = [mu[0], mu[1], std[0], std[1], 0, conf]
            ...
            if self._task_started:
                ...
                if self._log_wandb and self._wandb_run is not None:
                    wandb.log({
                        ...
                        "live/is_aligning_mode":     1.0 if self.controller.mode == 'aligning' else 0.0,
                        ...
                        "live/c4_dir_scale_0":       dir_scales[0] if len(dir_scales) > 0 else 1.0,
                        "live/c4_dir_scale_1":       dir_scales[1] if len(dir_scales) > 1 else 1.0,
                    })
                if self._goal_pos:
    ...
    def _compute_stable_dir_scales(self, n_obs: int, mu_list_list: list) -> list:
        _C4_ALPHA = 0.6
        _LP_ALPHA = 0.2   # low-pass: 80% previous, 20% new
        ...
        for Nn in range(n_obs):
            # Match this prediction slot to the closest raw trajectory by current position
            if not mu_list_list or Nn >= len(mu_list_list[0]):
                continue
            pred_x, pred_y = mu_list_list[0][Nn][0], mu_list_list[0][Nn][1]

            best_idx = -1
            best_dist = float('inf')
            for ti, traj in enumerate(raw_trajs):
                if not traj.traj_points:
                    continue
                lp = traj.traj_points[-1]
                d = math.hypot(pred_x - lp.x, pred_y - lp.y)
                if d < best_dist:
                    best_dist = d
                    best_idx = ti

            if best_idx < 0 or best_dist > 2.0:
                continue

            pts = raw_trajs[best_idx].traj_points
            if len(pts) < 5:
                continue

            # Stable heading: last point minus 5th-from-last (simple 5-step window)
            dx = pts[-1].x - pts[-5].x
            dy = pts[-1].y - pts[-5].y

            if abs(dx) < 1e-3 and abs(dy) < 1e-3:
                continue  # stationary pedestrian

            ped_heading = math.atan2(dy, dx)
            dtheta = math.atan2(
                math.sin(ped_heading - self.theta),
                math.cos(ped_heading - self.theta))
            raw_scale = min(1.0 + _C4_ALPHA * (1.0 - math.cos(dtheta)) / 2.0, 1.8)

            # Low-pass filter to suppress transient jitter
            prev = self._dir_scale_filtered.get(Nn, 1.0)
            filtered = (1.0 - _LP_ALPHA) * prev + _LP_ALPHA * raw_scale
