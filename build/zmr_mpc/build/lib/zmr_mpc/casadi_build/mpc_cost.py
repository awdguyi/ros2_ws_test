...
def cost_gaussian_obstacles(state: ca.SX, x_obs: ca.SX, y_obs: ca.SX,
                             alpha: ca.SX, sigma: float, weight: Union[ca.SX, float]=1.0) -> ca.SX:
    """Gaussian repulsion cost for dynamic obstacles.

    Provides non-zero gradient everywhere (unlike ellipse cost which has dead zone outside).
    Cost peaks at obstacle center and decays smoothly with distance.

    Args:
        state:   Robot state, x=state[0], y=state[1].
        x_obs:   Obstacle x-positions, shape (Ndynobs, 1).
        y_obs:   Obstacle y-positions, shape (Ndynobs, 1).
        alpha:   Per-obstacle confidence weights, shape (Ndynobs, 1).
        sigma:   Gaussian width in metres.
        weight:  Overall cost weight (q_dynobs).

    Returns:
        cost: (1,1) scalar cost.
    """
    dx = state[0] - x_obs          # (Ndynobs, 1)
    dy = state[1] - y_obs
    dist_sq = dx * dx + dy * dy
    gauss = ca.exp(-dist_sq / (2.0 * sigma ** 2))
    cost: ca.SX = weight * ca.sum1(alpha * gauss)
    assert cost.shape == (1, 1)
    return cost
